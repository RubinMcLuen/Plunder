...
const DEFAULT_ARROW_OFFSET := Vector2(1.0, -33.0)
...
func get_tutorial_state() -> Dictionary:
	...
	return {
		...
		"barnaby_hired": (barnaby.hired if is_instance_valid(barnaby) else "Barnaby" in Global.crew),
		"barnaby_pos": {
...
func apply_tutorial_state(state: Dictionary) -> void:
	...
	# If Barnaby has already been hired, free him from the scene
	if barnaby.hired:
...
func _on_bartender_dialogue_requested_tutorial(section: String) -> void:
	...
	# Step 1: still teaching movement, just show line
	if not stage_two_started:
		super._on_bartender_dialogue_requested(section)
		return

	# Step 3 already active → no change
	if stage_three_started:
		super._on_bartender_dialogue_requested(section)
		return

	# Transition to hiring step
	arrow.visible = false
	arrow.target = null
...
func _on_barnaby_dialogue_requested_tutorial(section: String, b: NPC) -> void:
	...
	balloon.connect("dialogue_finished", Callable(self, "_on_dialogue_finished_barnaby_tutorial").bind(b))
