...
func _process(delta: float) -> void:
	...
	for crew in crew_targets.keys():
		...
		# Debug: Print the positioning info
		if randf() < 0.05:  # Print occasionally
			print("Crew ", crew.npc_name, " (", side, " side) moving to: ", exact_target_position, " (enemy at: ", target_pos, ")")
		
		# Update target position for this crew
		crew_target_positions[crew] = exact_target_position
		
		# Check distance to exact position
		var distance_to_position = crew.global_position.distance_to(exact_target_position)
		
		# Debug current position
		if randf() < 0.05:  # Print occasionally
			print("Crew ", crew.npc_name, " at: ", crew.global_position, " distance to target: ", distance_to_position)
		
		# If very close (within 3 pixels), switch to combat - increased from 2 to help with precision
		if distance_to_position <= 3.0:
			# At exact position - switch to combat mode
		...
		var desired_velocity = direction * crew.speed * 0.6  # Slower for precision
